...
contract HustToken is IERC20 {
  ...
  function mint(address _to, uint256 _value) onlyOwner returns (bool success) {
    require(_to != address(0));
  ...
  function _transfer(address _from, address _to, uint256 _value) returns (bool success) {
    if (_from == address(0)) {
      // Minting new token.
